...
const ja: RankingCopy = {
  ...
  intro:
    'このページは編集部による選び方の目安で、順位が固定されたランキングではありません。まずは予算、滞在時間、希望エリア、宿泊の要否で候補を絞るのがおすすめです。料金、セラピストの勤務時間、営業状況は変わることがあります。表は当サイトの店舗データをもとに表示しており、一時休業中の店舗も資料として掲載されます。出発前に各詳細ページと当日の案内をご確認ください。',
  topPicks: {
